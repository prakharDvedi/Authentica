...
export async function generatePDFCertificate(
  ...
  doc.text("Certificate of Authenticity", pageWidth / 2, 35, {
    align: "center",
  });
  ...
  doc.setFontSize(24);
  doc.setFont("helvetica", "bold");
  doc.text("AI Artwork Proof of Creation", pageWidth / 2, yPos, {
    align: "center",
  });
  yPos += 15;
  ...
  doc.setFontSize(10);
  doc.text(
    cert.creator.substring(0, 42) + (cert.creator.length > 42 ? "..." : ""),
    margin + 25,
    yPos
  );
  yPos += 10;
  ...
  try {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...textColor);
    doc.text("QR Code", qrX + qrSize / 2, qrY + qrSize / 2, {
      align: "center",
    });
  }
  ...
  doc.text(
    "This certificate proves the authenticity and ownership of AI-generated artwork.",
    pageWidth / 2,
    footerY + 8,
    { align: "center" }
  );
  doc.text(
    "Stored immutably on blockchain and IPFS.",
    pageWidth / 2,
    footerY + 12,
    { align: "center" }
  );
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    `IPFS: ${cert.ipfsLink.substring(0, 50)}...`,
    pageWidth / 2,
    footerY + 16,
    { align: "center" }
  );
